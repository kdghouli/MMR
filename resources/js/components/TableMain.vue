<template>
    <div>
        ...
        <p class="pt-2">Nombre des <span class="fw-bolder"><span><i class="bi bi-truck text-success fs-4"></i></span>
                Camions:
            </span><span class="badge rounded-pill bg-success ms-1">{{ camionsMat.length }}</span> </p>

        <div class="row row-cols-lg-3 row-cols-md-2 ">

            <ul v-for="camion in camionsMat" class="list-group" :key="camion">

                <li class="list-group-item list-group-item-action d-flex justify-content-between align-items-center list-group-item-success mb-1 shadow-sm"
                    :class="{ ['bg-danger fw-bold text-white animate__animated animate__headShake animate__infinite']: calcComments(camion.id) }">
        ...
        <p class="pt-2">Nombre des <span class="fw-bolder"> Voitures: </span><span
                class="badge rounded-pill bg-primary ms-1">{{ voituresMat.length }}</span> </p>
        ...
        <p class="pt-2">Nombre des <span class="fw-bolder"><img class="me-1" src="" alt=""
                    style="width: 30px;" /> Chariots </span> <span class="badge rounded-pill bg-warning ms-1">{{
                        chariotsMat.length
                    }}
            </span> </p>
        ...
        <div class="row row-cols-lg-3 row-cols-md-2">
            <ul v-for="chariot in chariotsMat" class="list-group" :key="chariot">
                ...
                <li
                    ...
                    <router-link class="text-decoration-none" :to="{ name: 'ChariotView', params: { id: chariot.matricule } }">{{
                        chariot.matricule }}</router-link> <span class="fw-lighter ms-4 me-auto align-bottom text-truncate"
                        style="font-size: 9px">{{ chariot.marque }}</span><span
                        class="badge bg-warning rounded-pill">12</span>
                </li>
        ...
        <p class="pt-2">Nombre des <span class="fw-bolder"><img class="me-1" src="" alt=""
                    style="width: 30px;" /> Scooters : </span><span class="badge rounded-pill bg-danger ms-1">{{
                        scootersMat.length
                    }}</span> </p>
        ...
        <div class="row row-cols-lg-3 row-cols-md-2">
            <ul v-for="scooter in scootersMat" class="list-group" :key="scooter">
                ...
                <li
                    ...
                    <img class="me-1" src="" alt="" style="width: 22px;" />
                    <router-link class="text-decoration-none" :to="{ name: 'ScooterView', params: { id: scooter.matricule } }">{{
                        scooter.matricule }}</router-link> <span class="fw-lighter ms-4 me-auto align-bottom text-truncate"
                        style="font-size: 10px">{{ scooter.utilisateur }}</span><span
                        class="badge bg-danger rounded-pill">15</span>
                </li>
